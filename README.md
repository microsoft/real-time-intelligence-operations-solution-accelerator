...
This solution accelerator delivers comprehensive asset monitoring, real-time anomaly detection, and automated notifications for manufacturing operations. It uses synthetic telemetry data generated by the `Telemetry Data Simulator` to demonstrate capabilities. The architecture easily adapts to other industries by connecting your own data sources to `Azure Event Hub` and customizing the dashboards and queries. For technical details, see the [Solution Architecture Overview](./docs/TechnicalArchitecture.md).
